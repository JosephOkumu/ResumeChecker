...
router.get("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    // Check for feedback - prioritize JSON column from Gemini AI, fallback to normalized table
    let feedback = null;

    // First try to get feedback from the JSON column (Gemini AI)
    if (resume.feedback) {
      try {
        feedback = JSON.parse(resume.feedback);
        console.log("✅ Using feedback from JSON column (Gemini AI)");
      } catch (parseError) {
        console.warn(
          "❌ Failed to parse JSON feedback, trying normalized table"
        );
      }
    }

    // Fallback to normalized feedback table (Puter AI legacy)
    if (!feedback && resume.overall_score) {
      feedback = {
      ...
      console.log("✅ Using feedback from normalized table (legacy)");
    }
...
router.delete("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    await pool.execute("DELETE FROM resumes WHERE id = ? AND user_id = ?", [
      req.params.id,
      req.user.id,
    ]);
